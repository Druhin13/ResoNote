...
/**
 * @file Services for playlist generation, variation handling, diversity re-ranking,
 * and utility helpers for title normalization. Relies on track retrieval and
 * similarity computations provided by dataLoader and similarityService.
 */

/**
 * @typedef {Object} GenerationOptions
 * @property {number} [minTracks=5] Minimum number of tracks desired
 * @property {number} [maxTracks=10] Maximum number of tracks desired
 * @property {'semantic'|'audio'|'combined'} [similarityType='combined'] Similarity mode
 ...
 * @property {number} [diversityFactor=0.3] Trade-off between similarity and diversity in [0,1]
 * @property {boolean} [includeSeedTracks=true] Include seed tracks in the final playlist
 * @property {boolean} [allowTrackVariations=true] Allow near-duplicate versions/edits
 */

/**
 * @typedef {Object} TrackSummary
 * @property {string} track_id Track ID
 * @property {string} track_name Track title
 * @property {string} artist_name Artist name
 * @property {number} similarity Similarity score to seed or selected item
 * @property {boolean} [isSeed] Whether this is a seed track
 * @property {boolean} [isVariation] Marked as a variation of another track
 * @property {string} [variationOf] Seed track ID this item varies from
 * @property {string} [similarToSeed] Seed track ID this was matched against
 * @property {number} [combinedScore] Score used during diversity re-ranking
 * @property {boolean} [isVariationGroup] Indicates representative of a grouped set
 * @property {number} [groupSize] Count of grouped items for which this is the representative
 */

/**
 * @typedef {Object} PlaylistResult
 * @property {string} name Generated playlist title
 * @property {TrackSummary[]} tracks Ordered list of selected tracks
 * @property {{trackCount:number, seedTrackCount:number, averageSimilarity:number}} stats Aggregate statistics
 */
 ...
 * Applies candidate collection, de-duplication, optional variation filtering,
 * diversity re-ranking, and final truncation to bounds.
 *
 * @param {Array<string>} seedTrackIds Array of seed track IDs
 * @param {GenerationOptions} [options={}] Generation configuration
 * @returns {PlaylistResult} Generated playlist payload
 * @throws {Error} When no valid seeds are provided
 */
function generatePlaylist(seedTrackIds, options = {}) {
  ...
  for (const seedTrack of validSeedTracks) {
    const similarTracks = similarityService.findSimilarTracks(
      seedTrack.track_id,
      {
  ...
  for (const track of allSimilarTracks) {
    ...
    if (
      includeSeedTracks &&
      validSeedTracks.some((seedTrack) => seedTrack.track_id === trackId)
    ) {
      continue;
    }
  ...
  if (!allowTrackVariations) {
    ...
    for (const track of uniqueSimilarTracks) {
      ...
      for (const [groupKey, group] of groupedTracks.entries()) {
        ...
        const existingBaseName = getBaseTrackName(
          existingTrack.track_name || ""
        );
  ...
  let rankedTracks = applyDiversityReranking(
    uniqueSimilarTracks,
    diversityFactor
  );

  // Limit to maxTracks
  const recommendedTracks = rankedTracks.slice(
    0,
    maxTracks - playlistTracks.length
  );
  ...
  const totalSimilarity = playlistTracks.reduce(
    (sum, track) => sum + track.similarity,
    0
  );
...
/**
 * Extract a base track title by removing common variation markers
 * (remix/version/edit/mix/feat/live/etc.).
 *
...
/**
 * Re-rank tracks to balance relevance (similarity) and novelty (diversity).
 * Uses a greedy selection that maximizes a convex combination of similarity
 * and average dissimilarity to the already selected set.
 *
 * @param {TrackSummary[]} tracks Candidate tracks
 * @param {number} diversityFactor Mixing weight in [0,1] for diversity
 * @returns {TrackSummary[]} Re-ranked list
 */
function applyDiversityReranking(tracks, diversityFactor) {
  ...
  while (tracksToRank.length > 0) {
    ...
    for (const track of tracksToRank) {
      ...
      track.combinedScore =
        track.similarity * (1 - diversityFactor) +
...
/**
 * Build a human-readable playlist name from seed artists/titles.
 *
 * @param {TrackSummary[]} seedTracks Seed tracks included or referenced
...
function generatePlaylistName(seedTracks) {
  ...
  return `Mix of ${seedTracks[0].artist_name}, ${
    seedTracks[1].artist_name
  }, and ${seedTracks.length - 2} more`;
}

/**
 * Proxy to underlying similarity service for convenience/testing.
 *
 * @param {string} trackId Track ID to query from
 * @param {Object} [options] Similarity search options
 * @returns {TrackSummary[]} Similar tracks
 */
