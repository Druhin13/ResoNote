/**
 * @file Similarity computations over tracks, including semantic and audio measures,
 * a combined scorer with weighting, vector utilities, and a breakdown explainer.
 */
 ...
 *
 * Strategy:
 * - 'semantic': tag-based similarity (with confidence-weighted Jaccard and co-occurrence)
 * - 'audio': feature-space similarity (weighted, normalized distances)
 * - 'combined': convex combination of semantic and audio scores
 *
 * @param {string} trackId1 First track ID
 * @param {string} trackId2 Second track ID
 * @param {'semantic'|'audio'|'combined'} [similarityType='combined'] Similarity mode
 * @param {number} [semanticWeight=0.5] Weight for semantic component when combined
 * @param {number} [audioWeight=0.5] Weight for audio component when combined
 * @returns {number} Similarity score in [0,1]
 * @throws {Error} When similarityType is invalid
 */
function calculateTrackSimilarity(
  ...
  switch (similarityType) {
    ...
    case "combined":
      ...
      return (
        semantic * normalizedSemanticWeight + audio * normalizedAudioWeight
      );
    default:
...
/**
 * Calculate semantic similarity between two tracks based on tags and their confidences.
 * Combines facet-wise weighted similarity with cross-facet co-occurrence via cosine similarity.
 *
 * @param {Object} track1 First track data (expects tags/scores)
 * @param {Object} track2 Second track data (expects tags/scores)
 * @returns {number} Semantic similarity in [0,1]
 */
function calculateSemanticSimilarity(track1, track2) {
  ...
  for (const [facet, weight] of Object.entries(facetWeights)) {
    ...
    const scores1 =
      track1.scores && track1.scores[facet] ? track1.scores[facet] : {};
    const scores2 =
      track2.scores && track2.scores[facet] ? track2.scores[facet] : {};
  ...
  if (facetsWithTags >= 2) {
    ...
    const facetsWithData = Object.keys(facetWeights).filter(
      (facet) =>
        track1.tags[facet] &&
        track1.tags[facet].length > 0 &&
        track2.tags[facet] &&
        track2.tags[facet].length > 0
    );
  ...
  const combinedSimilarity =
    basicTagSimilarity * 0.4 + coOccurrenceSimilarity * 0.6;

  return combinedSimilarity;
}

/**
 * Build a sparse tag vector with single-tag entries and cross-facet co-occurrence features.
 *
 * @param {Object} track Track data with tags and optional scores
 * @param {Array<string>} facets Facets to include
 * @returns {Object<string, number>} Sparse vector keyed by feature name
 */
function buildTagVector(track, facets) {
  ...
  facets.forEach((facet) => {
    ...
    const scores =
      track.scores && track.scores[facet] ? track.scores[facet] : {};
    ...
    for (let j = i + 1; j < facets.length; j++) {
      ...
      tags1.forEach((tag1) => {
        tags2.forEach((tag2) => {
          ...
          const confidence1 =
            track.scores && track.scores[facet1]
              ? track.scores[facet1][tag1] || 1
              : 1;
          const confidence2 =
            track.scores && track.scores[facet2]
              ? track.scores[facet2][tag2] || 1
              : 1;
...
/**
 * Calculate audio similarity between two tracks using weighted, normalized feature differences.
 * Converts weighted distance to a similarity score.
 *
 * @param {Object} track1 First track data (expects features)
 * @param {Object} track2 Second track data (expects features)
 * @returns {number} Audio similarity in [0,1]
 */
function calculateAudioSimilarity(track1, track2) {
  ...
  for (const [feature, weight] of Object.entries(featureWeights)) {
    ...
    // Handle binary features
      ...
      const diff =
        track1.features[feature] === track2.features[feature] ? 0 : 1;
...
/**
 * Build a detailed similarity breakdown, including:
 * - Semantic per-facet scores and co-occurrence patterns
 * - Audio per-feature similarities and normalized values
 *
 * @param {Object} track1 First track data
 * @param {Object} track2 Second track data
 * @returns {Object} Breakdown with semantic and audio sections
 */
...
/**
 * Find similar tracks to a specific track by scanning the corpus and scoring candidates.
 *
 * @param {string} trackId ID of the source track
 * @param {Object} [options] Search options
 * @param {number} [options.limit=20] Max number of results
 * @param {'semantic'|'audio'|'combined'} [options.similarityType='combined'] Scoring mode
 * @param {number} [options.semanticWeight=0.5] Weight for semantic in combined mode
 * @param {number} [options.audioWeight=0.5] Weight for audio in combined mode
 * @param {number} [options.minSimilarity=0.1] Minimum similarity threshold
 * @returns {Array<{track_id:string, track_name:string, artist_name:string, similarity:number}>} Ranked results
 * @throws {Error} When the source track is not found
 */
